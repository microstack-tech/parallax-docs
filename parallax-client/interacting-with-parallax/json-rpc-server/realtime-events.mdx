...
The Parallax client supports publish / subscribe using JSON-RPC notifications. This allows clients to wait for events instead of polling for them.
...
2. Subscriptions require a full duplex connection. The Parallax client offers such connections in the form of WebSocket and IPC (enabled by default).
