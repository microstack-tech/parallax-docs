---
title: 'JS Console 2: Contracts'
---

The [Introduction to the Javascript console](./js-console) page outlined how a Javascript console can be attached to the Parallax client to provide a more user-friendly interface to Parallax than interacting directly with the JSON-RPC API. This page will describe how to deploy contracts and interact with contracts using the attached console. This page will assume the Javascript console is attached to a running the Parallax client instance using IPC. Clef should be used to manage accounts.

## Deploying a contract

First we need a contract to deploy. We can use the well-known `Storage.sol` contract written in Solidity. The following Solidity code can be copied and pasted into a text editor and saved as `parallax/storage-contract/Storage.sol`.
...
The contract needs to be compiled before the Parallax client can understand it. Compiling the contract creates an [Application Binary Interface](../tools/abi-gen) and the contract bytecode. This requires a Solidity compiler (e.g. `solc`) to be installed on the local machine. Then, compile and save the ABI and bytecode to a new `build` subdirectory using the following terminal commands:

```sh
cd ~/parallax/storage-contract
...
These are all the data required to deploy the contract using the Parallax client Javascript console. Open the Javascript console using `./prlx attach prlx.ipc`.
...
This page demonstrated how to create, compile, deploy and interact with a Parallax smart contract using the Parallax client's Javascript console.
