...
This document explains how to set up an [EIP 1459](https://eips.ethereum.org/EIPS/eip-1459) node list using the devp2p developer tool. The focus of this guide is creating a public list for the Parallax mainnet and public testnets, but it may also be helpful for setting up DNS-based discovery for a private network.
...
`cmd/devp2p` is a developer utility and is not included in the Parallax client distribution. You can install this command using go get:

```sh
go get github.com/microstack-tech/parallax/cmd/devp2p
```

To create a signing key, the `parallaxkey` utility is needed.

```sh
go get github.com/microstack-tech/parallax/cmd/parallaxkey
```
...
Then, to create the output set containing Parallax mainnet nodes only, run
...
* `-eth-network` ( `mainnet` | `testnet` ) selects a Parallax network.
...
To turn a node list into a DNS node tree, the list needs to be signed. To do this, a key pair is required. To create the key file in the correct format, the `cmd/parallaxkey` utility should be used. Choose a strong password to encrypt the key on disk!

```sh
parallaxkey generate dnskey.json
```
