...
There are also a set of tracers written in Javascript. These are less performant than the Go native tracers because of overheads associated with interpreting the Javascript in Parallax client's Go environment.
...
It is possible to give temporary state modifications to the Parallax client in order to simulate the effects of `eth_call`. For example, some new bytecode could be deployed to some address *temporarily just for the duration of the execution* and then a transaction interacting with that address can be traced. This can be used for scenario testing or determining the outcome of some hypothetical transaction before executing for real.
