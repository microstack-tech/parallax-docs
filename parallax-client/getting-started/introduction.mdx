This page explains how to set up the Parallax Client and execute some basic tasks using the command line tools. In order to use the Parallax Client, the software must first be installed. There are several ways the Parallax client can be installed depending on the operating system and the user's choice of installation method, for example using a package manager, container or building from source. Instructions for installing the Parallax client can be found on the ["Install and Build"](./installing-client) pages.

The tutorial on this page assumes the Parallax client have been installed successfully and that a firewall has been configured to block external traffic to the JSON-RPC port 8545, see [Security](../fundamentals/sync-modes.md).

This page provides step-by-step instructions covering the fundamentals of using the Parallax client. This includes generating accounts, joining the Parallax network, syncing the blockchain and sending laxes between accounts. This tutorial uses [Clef](../tools/clef/). Clef is an account management tool external to the Parallax client itself that allows users to sign transactions. It is developed by the Geth team and maintained by the Parallax team.
...
Users that need to revisit these fundamentals can find helpful resources relating to the command line [here](https://developer.mozilla.org/en-US/docs/Learn/Tools_and_testing/Understanding_client-side_tools/Command_line), http [here](https://developer.mozilla.org/en-US/docs/Web/HTTP) and Javascript [here](https://www.javascript.com/learn).

If the Parallax client was installed from source on Linux, make saves the binaries for the Parallax client and the associated tools in /build/bin. To run these programs it is convenient to move them to the top level project directory (e.g. running mv ./build/bin/\* ./) from /parallax. Then ./ must be prepended to the commands in the code snippets in order to execute a particular program, e.g. ./prlx instead of simply prlx. If the executables are not moved then either navigate to the bin directory to run them (e.g. cd ./build/bin and ./prlx) or provide their path (e.g. ./build/bin/prlx). These instructions can be ignored for other installations.
...
WARN [02-10|13:46:46.595] Please backup your key file!             path=keystore:///.../parallax-tutorial/keystore/UTC--2022-02-07T17-19-56.517538000Z--ca57f3b40b42fcce3c37b8d18adbca5260ca72ec
...
To start Clef, run the Clef executable passing as arguments the keystore file location, config directory location and a chain ID. The config directory was automatically created inside the parallax-tutorial directory during the previous step. The [chain ID](https://chainlist.org/) is an integer that defines which EVM compatible network to connect to. Parallax mainnet has **chain ID 2110**. In this tutorial Chain ID **2111** is used which is that of the Parallax testnet. It is very important that this chain ID parameter is set to 5201420 - Clef uses the chain ID to sign messages so it must be correct. The following command starts Clef on Parallax testnet:

```sh
clef --keystore parallax-tutorial/keystore --configdir parallax-tutorial/clef --chainid 5201420
```
...
WARN [02-10|13:55:30.947] Failed to open master, rules disabled    err="failed stat on parallax-tutorial/clef/masterseed.json: stat parallax-tutorial/clef/masterseed.json: no such file or directory"
INFO [02-10|13:55:30.947] Starting signer                          chainid=5201420 keystore=parallax-tutorial/keystore light-kdf=false advanced=false
...
INFO [02-10|13:55:30.984] IPC endpoint opened                      url=parallax-tutorial/clef/clef.ipc
------- Signer info -------
* intapi_version : 7.0.1
* extapi_version : 6.1.0
* extapi_http : n/a
* extapi_ipc : parallax-tutorial/clef/clef.ipc
```
...
If there is no error message reported to the terminal, everything is OK. The Parallax client must be running and synced in order for a user to interact with the Parallax network. If the terminal running the Parallax client is closed down then the Parallax client must be restarted again in a new terminal. The Parallax client can be started and stopped easily, but it must be running for any interaction with Parallax to take place. To shut down the Parallax client, simply press CTRL+C in the Parallax client terminal. To start it again, run the previous command prlx --datadir \<other commands>.

### Step 4: Get Testnet LAX

In order to make some transactions, the user must fund their account with Laxes. On the Parallax mainnet, Laxes can only be obtained in three ways:

1. by receiving it as a reward for mining blocks;
2. receiving it in a transfer from another Parallax user or contract;
3. receiving it from an exchange

On Parallax testnets, the Laxes has no real world value so it can be made freely available via faucets. Faucets allow users to request a transfer of testnet Laxes to their account.

### Step 5: Interact with the Parallax client

For interacting with the blockchain, the Parallax client provides JSON-RPC APIs. [JSON-RPC](../../parallax-protocol/parallax-stack/apis/json-rpc) is a way to execute specific tasks by sending instructions to the Parallax client in the form of [JSON](https://www.json.org/json-en.html) objects. RPC stands for "Remote Procedure Call" and it refers to the ability to send these JSON-encoded instructions from locations outside of those managed by the Parallax client. It is possible to interact with the Parallax client by sending these JSON encoded instructions directly over the Parallax client's exposed http port using tools like Curl. However, this is somewhat user-unfriendly and error-prone, especially for more complex instructions. For this reason, there are a set of libraries built on top of JSON-RPC that provide a more user-friendly interface for interacting with the Parallax client. One of the most widely used is Web3.js.

The Parallax client provides a Javascript console that exposes the Web3.js API. This means that with the Parallax client running in one terminal, a Javascript environment can be opened in another allowing the user to interact with the Parallax client using Web3.js. There are three transport protocols that can be used to connect the Javascript environment to the Parallax client:

* IPC (Inter-Process Communication): Provides unrestricted access to all APIs, but only works when the console is run on the same host as the Parallax node.
...
This tutorial will use the HTTP option. Note that the terminals running the Parallax client and Clef should both still be active. In a new (third) terminal, the following command can be run to start the console and connect it to the Parallax client using the exposed http port:

```sh
prlx attach http://127.0.0.1:8545
```
...
instance: Prlx/v1.0.0-stable/darwin-amd64/go1.24.0
at block: 6354736 (Thu Feb 10 2025 14:01:46 GMT+0100 (WAT))
 modules: eth:1.0 net:1.0 rpc:1.0 web3:1.0

To exit, press ctrl-d or type exit
```

The console is now active and connected to the Parallax client. It can now be used to interact with the Parallax (testnet) network.
...
You can select which accounts the caller can see
  [x] 0xca57F3b40B42FCce3c37B8D18aDBca5260ca72EC
    URL: keystore:///.../parallax-tutorial/keystore/UTC--2022-02-07T17-19-56.517538000Z--ca57f3b40b42fcce3c37b8d18adbca5260ca72ec
  [x] 0xCe8dBA5e4157c2B284d8853afEEea259344C1653
    URL: keystore:///.../parallax-tutorial/keystore/UTC--2022-02-10T12-46-45.265592000Z--ce8dba5e4157c2b284d8853afeeea259344c1653
-------------------------------------------
Request context:
        NA - ipc - NA
...
Having confirmed that the two addresses created earlier are indeed in the keystore and accessible through the Javascript console, it is possible to retrieve information about how much LAX they own. The testnet faucet should have sent 10 Laxes to the address provided, meaning that the balance of one of the accounts should be at least 10 Laxes and the other should be 0. There are other faucets available that may dispense more Laxes per request, and multiple requests can be made to accumulate more Laxes. The following command displays the account balance in the console:

```javascript
web3.fromWei(eth.getBalance('0xca57F3b40B42FCce3c37B8D18aDBca5260ca72EC'), 'ether');
```

There are actually two instructions sent in the above command. The inner one is the getBalance function from the eth namespace. This takes the account address as its only argument. By default, this returns the account balance in units of Wei. There are 1e-18 Wei to one LAX. To present the result in units of Laxes, getBalance is wrapped in the fromWei function from the web3 namespace. Running this command should provide the following result, assuming the account balance is 1 Lax:
...
**Send LAX to another account**

The command eth.sendTransaction can be used to send some Laxes from one address to another. This command takes three arguments: from, to and value. These define the sender and recipient addresses (as strings) and the amount of Wei to transfer. It is far less error prone to enter the transaction value in units of Lax rather than Wei, so the value field can take the return value from the toWei function. The following command, run in the Javascript console, sends 0.1 Lax from one of the accounts in the Clef keystore to the other.

Note that the addresses here are examples - the user must replace the address in the from field with the address currently owning 1 Lax, and the address in the to field with the address currently holding 0 Lax.
...
Note that submitting this transaction requires approval in Clef. In the Clef terminal, Clef will prompt for approval and request the account password. If the password is correctly entered, the Parallax client proceeds with the transaction. The transaction request summary is presented by Clef in the Clef terminal. This is an opportunity for the sender to review the details and ensure they are correct.
...
It is also advised to check the account balances using the Parallax client by repeating the instructions from earlier. At this point in the tutorial, the balances of the two accounts in the Clef keystore should have changed by \~0.1 Lax (the sender's balance will have decremented by a little over 0.1 Lax because some small was amount paid in transaction gas).

**Checking the transaction hash**

The transaction hash is a unique identifier for this specific transaction that can be used later to retrieve the transaction details. The same information can also be retrieved directly from the Parallax client. The hash returned in the previous step can be provided as an argument to eth.getTransaction to return the transaction information:
...
The balance is in the result field in the returned JSON object. However, it is denominated in Wei and presented as a hexadecimal string. There are many options for converting this value to a decimal in units of Lax, for example by opening a Python console and running:

```python
0xc7d54951f87f7c0 / 1e18
```

This returns the balance in Lax:
...
Sending a transaction between accounts can also be achieved using Curl. Notice that the value of the transaction is a hexadecimal string in units of Wei. To transfer 0.1 LAX, it is first necessary to convert this to Wei by multiplying by 10^18 then converting to hex. 0.1 LAX is "0x16345785d8a0000" in hex. As before, update the to and from fields with the addresses in the Clef keystore.
...
This tutorial has demonstrated how to generate accounts using Clef, fund them with testnet Laxes and use those accounts to interact with Parallax (testnet) through the Parallax client. Checking account balances, sending transactions and retrieving transaction details were explained using the web3.js library via the Parallax client console and using the JSON-RPC directly using Curl. For more detailed information about Clef, please see [the Clef docs](../tools/clef/).
