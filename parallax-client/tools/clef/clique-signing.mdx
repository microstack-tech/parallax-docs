...
It is useful to have basic knowledge of private networks and Clef. These topics are covered on our [private networks](../../fundamentals/private-networks) and [Introduction to Clef](./introduction) pages.
...
Initiate the Parallax client:

```sh
prlx  --datadir ./ddir init genesis.json
```
...
At this point the Parallax client has been initiated with a genesis configuration.
...
then start the Parallax client:

```sh
prlx  --datadir ./ddir --signer ./clef/clef.ipc --mine
```

The Parallax client will ask what accounts are present - enter `y` to approve:
...
After this, the Parallax client will start asking clef to sign things:
...
And indeed, after approving with `y`, the password is not required - the signed block is returned to the Parallax client:
...
Once the Parallax client starts asking clef to seal blocks, the data will be displayed. From that data, rules can be defined that allow signing clique headers but nothing else.
...
For the Parallax client, the only change is to provide `--signer <path to clef ipc>`.

## Summary

Clef can be used as a signer that automatically seals Clique blocks. This is a much more secure option than unlocking accounts using the Parallax client's built-in account manager.
