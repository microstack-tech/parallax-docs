This page provides a step-by-step walkthrough tutorial demonstrating some common uses of Clef. This includes manual approvals and automated rules. Clef is presented both as a standalone general signer with requests made via RPC and also as a backend signer for the Parallax client.
...
This tutorial will use Clef with the Parallax client on testnet. The accounts used will be in the testnet keystore with the path `~/parallax/testnet/keystore`. The tutorial assumes there are two accounts in this keystore. Instructions for creating accounts can be found on the [Account managament page](../../fundamentals/account-management). Note that Clef can also interact with hardware wallets, although that is not demonstrated here.

Clef should be started before the Parallax client, otherwise the client will complain that it cannot find a Clef instance to connect to. Clef should be started with the correct chainid for testnet. Clef itself does not connect to a blockchain, but the chainID parameter is included in the data that is aggregated to form a signature. Clef also needs a path to the correct keystore passed to the `--keystore` command. A custom path to the config directory can also be provided. This is where the ipc file will be saved which is needed to connect Clef to the Parallax client:

```sh
clef --keystore ~/parallax/testnet/keystore --configdir ~/parallax/testnet/clef --chainid=2111
```

The following logs will be displayed in the console:

```
INFO [07-01|11:00:46.385] Starting signer                          chainid=5201420 keystore= parallax/testnet/keystore light-kdf=false advanced=false
...
INFO [07-01|11:00:46.392] IPC endpoint opened                      url=parallax/testnet/clef/clef.ipc
------- Signer info -------
* intapi_version : 7.0.1
* extapi_version : 6.1.0
* extapi_http : n/a
* extapi_ipc : parallax/testnet/clef/clef.ipc
```
...
The code snippet below shows a request made to Clef via its *External API endpoint* using [NetCat](http://netcat.sourceforge.net/). The request invokes the ["account\_list"](./apis#account_list) endpoint which lists the accounts in the keystore. This command should be run in a new terminal.
...
You can select which accounts the caller can see
  [x] 0xD9C9Cd5f6779558b6e0eD4e6Acf6b1947E7fA1F3
    URL: keystore://parallax/testnet/keystore/UTC--2017-04-14T15-15-00.327614556Z--d9c9cd5f6779558b6e0ed4e6acf6b1947e7fa1f3
  [x] 0x086278A6C067775F71d6B2BB1856Db6E28c30418
    URL: keystore://parallax/testnet/keystore/UTC--2018-02-06T22-53-11.211657239Z--086278a6c067775f71d6b2bb1856db6e28c30418
-------------------------------------------
Request context:
 NA - ipc - NA
...
Apart from listing accounts, a *request* can be submitted to create a new account, signing transactions and data or recovering signatures. The available methods are documented in the Clef [External API Spec](https://github.com/microstack-tech/parallax/tree/main/cmd/clef#external-api-1) and the [External API Changelog](https://github.com/microstack-tech/parallax/blob/main/cmd/clef/extapi_changelog.md).

<Note>
*Note, the number of things that can be done from the External API is deliberately small to limit the power of remote calls as much as possible! Clef has an* [*Internal API*](https://github.com/microstack-tech/parallax/tree/main/cmd/clef#ui-api-1) *too for the UI (User Interface) which is much richer and can support custom interfaces on top. But that's out of scope here.*
</Note>

The example above used Clef completely independently of the Parallax client. However, by defining Clef as the signer when the Parallax client is started imposes Clef's `request` - `confirm` - `result` pattern to any interaction with the local Parallax node that touches accounts, including requests made using RPC or an attached Javascript console. To demonstrate this, the Parallax client can be started, with Clef as the signer:

```sh
prlx --testnet --datadir testnet --signer=testnet/clef/clef.ipc
```

With the Parallax client running, open a new terminal and attach a Javascript console:

```sh
prlx attach testnet/prlx.ipc
```
...
Approving this transaction causes Clef to prompt the user to provide the password for the sender account. Providing the password enables the transaction to be signed and sent to the Parallax client for broadcasting to the network. The details of the signed transaction are displayed in the console. Account passwords can also be stored in Clef's encrypted vault so that they do not have to be manually entered - [more on this below](#account-passwords).
...
* Auto-approve transactions with `Uniswap v2`, with value between `0.1` and `0.5 Lax` per 24h period
...
Rules are implemented as Javascript code in js files. The ruleset engine includes the same methods as the JSON\_RPC defined in the [UI Protocol](./datatypes). The following code snippet demonstrates a rule file that approves a transaction if it satisfies the following conditions:
...
```sh
clef --keystore parallax/testnet/ --configdir parallax/testnet/clef --chainid 5201420 --rules rules.js
```
...
INFO [07-01|13:39:49.726] Starting signer                          chainid=5201420 keystore=$parallax/testnet/ light-kdf=false advanced=false
...
INFO [07-01|13:39:49.728] IPC endpoint opened                      url=parallax/testnet/clef/clef.ipc
------- Signer info -------
...
**6)** Make requests directly to Clef using the external API or connect to the Parallax client by passing `--signer=<path to clef.ipc>` at client startup
...
```sh
clef --keystore parallax/testnet/clef --configdir parallax/testnet/clef --chainid 5201420 --rules rules.js
```

```
INFO [07-01|14:12:41.636] Rule engine configured                   file=rules.js
INFO [07-01|14:12:41.636] Starting signer                          chainid=5201420 keystore=parallax/testnet/clef/keystore light-kdf=false advanced=false
...
INFO [07-01|14:12:41.638] IPC endpoint opened                      url=parallax/testnet/clef/clef.ipc
------- Signer info -------
* intapi_version : 7.0.0
* extapi_version : 6.0.0
* extapi_http : n/a
...
Finally, a request can be submitted to test that the rules are being applied as expected. Here, Clef is used independently of the Parallax client by making a request via RPC, but the same logic would be imposed if the request was made via a connected Parallax node. Some arbitrary text will be included in the message data that includes the term `wen-merge`. The plaintext `clefdemotextthatincludeswen-merge` is `636c656664656d6f7465787474686174696e636c7564657377656e2d6d65726765` when represented as a hexadecimal string. This can be passed as data to an `account_signData` request as follows:

```sh
echo '{"id": 1, "jsonrpc":"2.0", "method":"account_signData", "params":["data/plain", "0x636c656664656d6f7465787474686174696e636c7564657377656e2d6d65726765"]}' | nc -U ~/parallax/testnet/clef/clef.ipc
```
...
```sh
echo '{"id": 1, "jsonrpc":"2.0", "method":"account_signData", "params":["data/plain", "0x636c656664656d6f74657874776974686f75747370656369616c74657874"]}' | nc -U ~/parallax/testnet/clef/clef.ipc
```
...
More examples, including a ruleset for a rate-limited window, are available on the [Clef GitHub](https://github.com/microstack-tech/parallax/blob/main/cmd/clef/rules.md#example-1-ruleset-for-a-rate-limited-window) and on the [Rules page](./rules).
...
## Parallax client integration

This tutorial has bounced back and forth between demonstrating Clef as a standalone tool by making 'manual\` JSON RPC requests from the terminal and integrating it as a backend singer for the Parallax client. Using Clef for account management is considered best practise for the Parallax client users because of the additional security benefits it offers over and above what it offered by the Parallax client's built-in accounts module. Clef is far more flexible and composable than the Parallax client's built-in account management tool and can interface directly with hardware wallets, while Apps and wallets can request signatures directly from Clef.

Ultimately, the goal is to deprecate the Parallax client's account management tools completely and replace them with Clef. Until then, users are simply encouraged to choose to use Clef as an optional backend signer for the Parallax client. In addition to the examples on this page, the [Getting started tutorial](../../getting-started/introduction) also demonstrates Clef/Parallax client integration.

## Summary

This page includes step-by-step instructions for basic and intermediate uses of Clef, including using it as a standalone app and a backend signer for the Parallax client. Further information is available on our other Clef pages, including [Introduction](./introduction), [Setup](./setup), [Rules](./rules), [Communication Datatypes](./datatypes) and [Communication APIs](./apis).
