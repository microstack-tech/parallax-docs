There are several ways to monitor the performance of a Parallax node. Insights into a node's performance are useful for debugging, tuning and understanding what is really happening when the Parallax client is running.

## Prerequisites

To follow along with the instructions on this page it will be useful to have:

* a running Parallax client instance.
...
The Parallax client collects lots of data which can be read in the form of a chronological database. To make monitoring easier, this data can be fed into data visualisation software. On this page, the Parallax client will be configured to push data into a InfluxDB database and Grafana will be used to visualise the data.
...
A database and user for Parallax client metrics can be created by communicating with it directly via its shell.

```sh
create database prlx
create user prlx with password choosepassword
```
...
InfluxDB is running and configured to store metrics from the Parallax client
...
## Preparing the Parallax client

After setting up database, metrics need to be enabled in the Parallax client. Various options are available, as documented in the `METRICS AND STATS OPTIONS` in `prlx --help` and in our [metrics page](../monitoring/metrics). In this case the Parallax client will be configured to push data into InfluxDB. Basic setup specifies the endpoint where InfluxDB is reachable and authenticates the database.

```sh
prlx --metrics --metrics.influxdb --metrics.influxdb.endpoint "http://0.0.0.0:8086" --metrics.influxdb.username "prlx" --metrics.influxdb.password "chosenpassword"
```

These flags can be provided when the Parallax client is started or saved to the configuration file.

Listing the metrics in the database verifies that the Parallax client is pushing data correctly. In InfluxDB shell:
...
With the InfluxDB database setup and successfully receiving data from the Parallax client the next step is to install Grafana so that the data can be visualized.
...
For a Parallax client InfluxDB monitoring dashboard, copy the URL of [this dashboard](https://grafana.com/grafana/dashboards/13877/) and paste it in the "Import page" in Grafana. After saving the dashboard, it should look like this:

<img src="https://geth.ethereum.org/images/docs/grafana.png" alt=""/>

For a Parallax client Prometheus monitoring dashboard, copy the URL of [this dashboard](https://grafana.com/grafana/dashboards/18463/) and paste it in the "Import page" in Grafana. After saving the dashboard, it should look like this:

<img src="https://geth.ethereum.org/images/docs/grafana2.png" alt=""/>
