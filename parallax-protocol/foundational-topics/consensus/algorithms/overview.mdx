...
Ethash is the mining algorithm that is actually used on Parallax. Ethash is effectively a new name given to a specific version of Dagger-Hashimoto after the algorithm got significantly updated, whilst still inheriting the fundamental principles of its predecessor.
