...
- Installed GPU mining software such as [Ethminer](../mining-with-ethminer-gpu).  
...
<Steps>
  ...
  <Step title="Run your miner">
    Use Ethminer (or another Ethash-compatible miner) to connect to the pool.

    Example command with Ethminer:

    ```bash
    ethminer -P stratum+tcp://<YOUR_ADDRESS>@<POOL_URL>:4444
    ```
  ...
  <Step title="Confirm shares are submitted">
    ...
    ```
    m  16:12:42|ethminer  Accepted share #42
    ```
